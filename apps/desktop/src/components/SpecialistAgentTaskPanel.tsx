...
type Props = { tool: Tool; disabled: boolean; canPersist: boolean; onActivity: () => Promise<void> | void };

const priorityLabels: Record<TaskPriority, string> = { hoy: "Hoy", "esta-semana": "Esta semana", profundizar: "Profundizar" };

export default function SpecialistAgentTaskPanel({ tool, disabled, canPersist, onActivity }: Props) {
  ...
  useEffect(() => {
    const memory = canPersist ? readAgentMemory(tool.agent.id) : null;
    setSavedGoal(memory?.goal ?? "");
    setTask(memory?.goal ?? "");
    setPlan(null);
    setRememberGoal(false);
  }, [tool.agent.id, canPersist]);
  ...
  return <section className="agent-task-panel" aria-label={`Plan de ${tool.agent.name}`}>
    ...
    <div className="agent-task-controls"><label>Prioridad<select value={priority} onChange={(event) => setPriority(event.target.value as TaskPriority)} disabled={disabled || busy}>{(Object.keys(priorityLabels) as TaskPriority[]).map((item) => <option key={item} value={item}>{priorityLabels[item]}</option>)}</select></label><label className="checkbox-row"><input type="checkbox" checked={includeProfile} onChange={(event) => setIncludeProfile(event.target.checked)} disabled={disabled || busy || !canPersist} /> Usar perfil personal local</label><label className="checkbox-row"><input type="checkbox" checked={rememberGoal} onChange={(event) => setRememberGoal(event.target.checked)} disabled={disabled || busy || !canPersist} /> Conservar objetivo</label></div>
    {canPersist && savedGoal && <div className="saved-agent-goal"><span>Objetivo guardado localmente</span><button type="button" onClick={removeSavedGoal}>Eliminar</button></div>}
