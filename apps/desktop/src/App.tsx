...
function App() {
  ...
  useEffect(() => { sessionApi.get().then((value) => value.access ? (setSession(value), setServiceOnline(true)) : beginGuest()).catch(beginGuest); }, []);
  useEffect(() => { if (isAdministrator) setDrafts(loadDrafts()); else setDrafts([]); }, [isAdministrator]);
  useEffect(() => { const tool = tools.find((item) => item.id === routedToolId); if (tool) setActiveTool(tool); }, [routedToolId]);
  ...
  const submitAdmin = async (event: FormEvent) => {
    ...
    try {
      ...
      setSession(value); setServiceOnline(true); setAccessCode(""); setAccessOpen(false);
    } catch (error) { setLoginError(error instanceof Error ? error.message : "No se pudo validar el código de administrador."); }
    finally { setLoginBusy(false); }
  };
  const returnToGuest = async () => { try { await sessionApi.logout(); } finally { await beginGuest(); setAgentSettingsOpen(false); } };
